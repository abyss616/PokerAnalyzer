...
public sealed class PreflopRegretTrainerTests
{
    ...
    public void RunIteration_DoesNotUpdateRegretsForChanceOrLeafNodes()
    {
        ...
        // No chance/leaf infosets are written since only action nodes update regrets.
    ...
    public void RunIteration_ReplaysActionsFromStateBeforeAction()
    {
        ...
        Assert.All(traverser.RolloutRoots, state => Assert.Equal(1, state.ActionHistory.Count));
    }
    ...
    private sealed class StubTrajectoryTraverser : IPreflopTrajectoryTraverser
    {
        ...
        {
            if (rootState.ActionHistory.Count == 0)
            {
            ...
            var firstAction = rootState.ActionHistory[0].ActionType;
            var utility = firstAction == ActionType.Fold ? 10d : 4d;
            if (rootState.ActionHistory.Count > 1)
                utility = 100d;
