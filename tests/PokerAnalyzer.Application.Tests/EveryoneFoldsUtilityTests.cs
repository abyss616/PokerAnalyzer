...
public sealed class EveryoneFoldsUtilityTests
{
    ...
    [Fact]
    public void ComputeEveryoneFoldsUtility_WithNegativeContribution_Throws()
    {
        var contributed = new decimal[] { 1m, -2m, 3m };
        var folded = new[] { true, false, true };

        var ex = Assert.Throws<ArgumentOutOfRangeException>(() =>
            TerminalUtilities.ComputeEveryoneFoldsUtility(contributed, folded, rake: 0m));

        Assert.Equal("contributed", ex.ParamName);
    }
}